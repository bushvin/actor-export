...
const fileName = 'pf2e-remastered.pdf';
...
mapper.textBox('heropoint', fileName, 0, 510, 36, 9, 16, pf2eHelper.evalCheckMark(heroPoints >= 1), mf_17_centered);
mapper.textBox('heropoint', fileName, 0, 533, 36, 9, 16, pf2eHelper.evalCheckMark(heroPoints >= 2), mf_17_centered);
mapper.textBox('heropoint', fileName, 0, 556, 36, 9, 16, pf2eHelper.evalCheckMark(heroPoints >= 3), mf_17_centered);
...
Object.values(character.defenseProficiencies)
    ...
    .forEach((d, i) => {
        ...
        mapper.textBox(ref, fileName, 0, x, 244, 7, 5, pf2eHelper.evalCheckMark(d.rank >= 1), mf_10_centered);
        mapper.textBox(ref, fileName, 0, x, 250, 7, 5, pf2eHelper.evalCheckMark(d.rank >= 2), mf_10_centered);
        mapper.textBox(ref, fileName, 0, x, 257, 7, 5, pf2eHelper.evalCheckMark(d.rank >= 3), mf_10_centered);
        mapper.textBox(ref, fileName, 0, x, 264, 7, 5, pf2eHelper.evalCheckMark(d.rank >= 4), mf_10_centered);
    });
...
Object.values(character.savingThrows).forEach((s, i) => {
    ...
    mapper.textBox(ref, fileName, 0, x + 47, 205, 5, 4, pf2eHelper.evalCheckMark(s.rank >= 1), mf_10_centered);
    mapper.textBox(ref, fileName, 0, x + 47, 209, 5, 4, pf2eHelper.evalCheckMark(s.rank >= 2), mf_10_centered);
    mapper.textBox(ref, fileName, 0, x + 47, 215, 5, 4, pf2eHelper.evalCheckMark(s.rank >= 3), mf_10_centered);
    mapper.textBox(ref, fileName, 0, x + 47, 220, 5, 4, pf2eHelper.evalCheckMark(s.rank >= 4), mf_10_centered);
});
...
mapper.textBox('dying', fileName, 0, 555, 215, 4, 4, pf2eHelper.evalCheckMark(dyingValue >= 1), mf_10_centered);
mapper.textBox('dying', fileName, 0, 564, 215, 4, 4, pf2eHelper.evalCheckMark(dyingValue >= 2), mf_10_centered);
mapper.textBox('dying', fileName, 0, 572, 215, 4, 4, pf2eHelper.evalCheckMark(dyingValue >= 3), mf_10_centered);
mapper.textBox('dying', fileName, 0, 580, 215, 4, 4, pf2eHelper.evalCheckMark(dyingValue >= 4), mf_10_centered);
...
mapper.textBox('perception', fileName, 0, 424, 297, 5, 5, pf2eHelper.evalCheckMark(pRank >= 1), mf_8_centered);
mapper.textBox('perception', fileName, 0, 424, 302, 5, 5, pf2eHelper.evalCheckMark(pRank >= 2), mf_8_centered);
mapper.textBox('perception', fileName, 0, 424, 308, 5, 5, pf2eHelper.evalCheckMark(pRank >= 3), mf_8_centered);
mapper.textBox('perception', fileName, 0, 424, 313, 5, 5, pf2eHelper.evalCheckMark(pRank >= 4), mf_8_centered);
...
character.strikes.forEach((strike) => {
    ...
    if (typeof y !== 'undefined') {
        ...
        mapper.textBox(ref, fileName, 0, 576, y, 5, 5, pf2eHelper.evalCheckMark(hasBludgeoningDamage), mf_8_centered);
        mapper.textBox(ref, fileName, 0, 576, y + 6, 5, 5, pf2eHelper.evalCheckMark(hasPiercingDamage), mf_8_centered);
...
['unarmed', 'simple', 'martial', 'advanced'].forEach((a) => {
    ...
    mapper.textBox('weapon proficiencies', fileName, 0, x, 672, 6, 6, pf2eHelper.evalCheckMark(wp >= 1), mf_8_centered);
    mapper.textBox('weapon proficiencies', fileName, 0, x, 679, 6, 6, pf2eHelper.evalCheckMark(wp >= 2), mf_8_centered);
    mapper.textBox('weapon proficiencies', fileName, 0, x, 685, 6, 6, pf2eHelper.evalCheckMark(wp >= 3), mf_8_centered);
    mapper.textBox('weapon proficiencies', fileName, 0, x, 692, 6, 6, pf2eHelper.evalCheckMark(wp >= 4), mf_8_centered);
});
...
if (character.bonusFeats.length > 0) {
    ...
    mapper.textBox('Bonus Feats', fileName, 1, 216, 108, 179, 58, bonusFeats, mf_8_multiline);
}
...
character.flatItems(character.wornItems).forEach((item) => {
    if (y <= 580) {
        ...
        mapper.textBox(ref, fileName, 1, 530, y, 27, 10, pf2eHelper.evalCheckMark(item.isInvested), mf_8_centered);
        mapper.textBox(ref, fileName, 1, 566, y, 14, 10, item.bulk, mf_8_centered);
        y = y + 10;
    }
});
...
character.activities.forEach((a) => {
    let y;
    let x;
    if (a.type === 'action' && actionActivitiesY.length > 0) {
        y = actionActivitiesY[0];
        actionActivitiesY.shift();
        x = 27;
        ref = 'action';
    } else if (['free', 'reaction'].includes(a.type) && freeActionActivitiesY.length > 0) {
        y = freeActionActivitiesY[0];
        freeActionActivitiesY.shift();
        x = 311;
        ref = 'free/reaction';
    }
    if (typeof y !== 'undefined') {
        ...
        mapper.textBox(ref, fileName, 2, x, y, 25, 12, a.glyph, action_10);
        mapper.textBox(ref, fileName, 2, x + 25, y, 202, 12, a.name, mf_10);
        mapper.textBox(ref, fileName, 2, x + 227, y, 50, 12, reference, mf_10);

        mapper.textBox(ref, fileName, 2, x + 25, y + 12, 202, 20, traits, mf_8_multiline);
        mapper.textBox(ref, fileName, 2, x + 227, y + 12, 50, 20, a.frequency, mf_8_multiline);
    }
});
...
mapper.textBox(ref, fileName, 3, 64, 41, 10, 10, pf2eHelper.evalCheckMark(hasArcaneTradition), mf_10_centered);
mapper.textBox(ref, fileName, 3, 72, 41, 10, 10, pf2eHelper.evalCheckMark(hasOccultTradition), mf_10_centered);
mapper.textBox(ref, fileName, 3, 64, 51, 10, 10, pf2eHelper.evalCheckMark(hasPrimalTradition), mf_10_centered_top);
mapper.textBox(ref, fileName, 3, 72, 51, 10, 10, pf2eHelper.evalCheckMark(hasDivineTradition), mf_10_centered_top);
...
mapper.textBox(ref, fileName, 3, 129, 46, 5, 5, pf2eHelper.evalCheckMark(isPreparedCaster), mf_8_centered);
mapper.textBox(ref, fileName, 3, 129, 56, 5, 5, pf2eHelper.evalCheckMark(isSpontaneousCaster), mf_8_centered);
...
if (typeof spellProficiency !== 'undefined') {
    ...
    mapper.textBox(ref, fileName, 3, 57, 100, 5, 5, pf2eHelper.evalCheckMark(spellAttackRank >= 1), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 57, 105, 5, 5, pf2eHelper.evalCheckMark(spellAttackRank >= 2), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 57, 111, 5, 5, pf2eHelper.evalCheckMark(spellAttackRank >= 3), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 57, 116, 5, 5, pf2eHelper.evalCheckMark(spellAttackRank >= 4), mf_8_centered);
    ...
    mapper.textBox(ref, fileName, 3, 142, 100, 5, 5, pf2eHelper.evalCheckMark(spellSaveRank >= 1), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 142, 105, 5, 5, pf2eHelper.evalCheckMark(spellSaveRank >= 2), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 142, 111, 5, 5, pf2eHelper.evalCheckMark(spellSaveRank >= 3), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 142, 116, 5, 5, pf2eHelper.evalCheckMark(spellSaveRank >= 4), mf_8_centered);
}
// cantrips
// cantrips per day
...
mapper.textBox(ref, fileName, 3, 80, 467, 12, 12, pf2eHelper.evalCheckMark(focusPointsMax >= 1), mf_10_centered);
mapper.textBox(ref, fileName, 3, 93, 467, 12, 12, pf2eHelper.evalCheckMark(focusPointsMax >= 2), mf_10_centered);
mapper.textBox(ref, fileName, 3, 106, 467, 12, 12, pf2eHelper.evalCheckMark(focusPointsMax >= 3), mf_10_centered);
    ...
    .forEach((spell) => {
        ...
        if (typeof y !== 'undefined') {
            if (sceSpellCount > 1 && sceType !== spell.type) {
                ...
                mapper.textBox(ref, fileName, x, 30, y, 169, 12, spell.type, mf_10);
                sceType = spell.type;
                y = spellY;
                spellY = spellY + 10;
                spellRank = undefined;
