...
const class_id = actor.class?._id || 'unknown';
...
spellCastingEntries
    ...
    .forEach((sce) => {
        ...
        const cantrips = sce.spells
            .filter((i) => i.isCantrip)
            .sort((a, b) => {
                return a.name < b.name ? -1 : a.name > b.name ? 1 : 0;
            });
        if (!hasCantripTitle && spellCastingEntries.length > 0 && cantrips.length > 0) {
            mapper.field('all', `cantrip_entry${cantrip_count++}_name`, sce.name);
            hasCantripTitle = true;
        }
        cantrips.forEach((s) => {
            let spell_name = s.name;
            let spell_actions = pf2eHelper.formatSpellCastingTime(s.system.time.value, pf2eHelper.pdfActionIconsGlyphs);
            let spell_rank = s.rank;
            let spell_prep = Object.values(sce.system.slots[`slot0`].prepared).filter((i) => i.id === s._id).length;

            mapper.field('all', `cantrip_entry${cantrip_count}_name`, spell_name);
            mapper.field('all', `cantrip_entry${cantrip_count}_actions`, spell_actions);
            mapper.field('all', `cantrip_entry${cantrip_count}_rank`, spell_rank);
            mapper.field('all', `cantrip_entry${cantrip_count++}_prep`, spell_prep === 0 ? '' : spell_prep);
        });
        if (spellCastingEntries.length > 0 && cantrips.filter((i) => i.isCantrip).length > 0) {
